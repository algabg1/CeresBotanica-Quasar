...
async function addPlantsToProject () {
  ...
  try {
    ...
    await api.post('http://localhost:8080/projeto-planta/adicionar-planta-projeto', plantsToAdd, {
      headers: { Authorization: `Bearer ${token}` }
    })
...
async function removePlant () {
  try {
    const token = localStorage.getItem('userToken')
    await api.delete(`http://localhost:8080/projeto-planta/remover-planta/${plantToRemove.value.id}`, {
      headers: { Authorization: `Bearer ${token}` }
    })
...
async function fetchProjectPlants (projectId) {
  try {
    const token = localStorage.getItem('userToken')
    const response = await api.get(`http://localhost:8080/projeto-planta/minhas-plantas/${projectId}`, {
      headers: { Authorization: `Bearer ${token}` }
    })
